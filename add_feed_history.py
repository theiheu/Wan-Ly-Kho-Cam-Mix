...
def add_feed_history_table():
    ...
    setup_tab_pattern = r'def setup_feed_usage_tab\(self\):.*?(?=def \w+\(self\):)'
    ...
    # Check if the feed_usage_history_table already exists
    if 'self.feed_usage_history_table' in current_method:
        print("Feed usage history table already exists")
        return

    # Modify the method to add the feed usage history table
    new_method = current_method.replace(
        # Find the part where we add the feed_table to the layout
    ...
    # Add the required methods for handling feed usage history
    history_methods = '''
    ...
    if save_report_match:
        ...
        if 'total_feed' in current_save_report and 'total_mix' in current_save_report and 'batch_count' in current_save_report:
            print("save_report method already has the required fields")
        else:
    ...
    # Replace the setup_feed_usage_tab method with the new one
    content = content.replace(current_method, new_method)

    # Add the history methods to the class
    # Find a good spot to add the methods (before the main function)
    ...
    if main_function_match:
        ...
        content = content[:insert_position] + history_methods + content[insert_position:]

    # Write the modified content back to the file
    with open('src/main.py', 'w', encoding='utf-8') as f:
        f.write(content)

    print("Successfully added feed usage history table to main.py")
